...
def generate_applications_section(task_data):
    ...
    html += '<div style="max-height: 600px; overflow-y: auto; border: 1px solid #bae6fd; border-radius: 4px; background: white;">\n'
    html += '<table style="width: 100%; border-collapse: collapse; font-size: 0.85em;">\n'
    html += '<thead style="position: sticky; top: 0; background: #e0f2fe; z-index: 1;">\n'
    html += '<tr>\n'
    ...
    for idx, app in enumerate(apps):
        ...
        if len(parts) >= 4:
            name = parts[0].strip()
            version = parts[1].strip().replace('v', '').replace('Version:', '').strip() if len(parts) > 1 else 'N/A'
            publisher = parts[2].strip().replace('Publisher:', '').strip() if len(parts) > 2 else 'N/A'
            date = parts[3].strip().replace('Installed:', '').strip() if len(parts) > 3 else 'N/A'
            arch = parts[4].strip().replace('Arch:', '').strip() if len(parts) > 4 else 'N/A'
            
            bg_color = '#ffffff' if idx % 2 == 0 else '#f0f9ff'
            html += f'<tr style="background: {bg_color}; transition: background 0.2s;" onmouseover="this.style.background=\'#bae6fd\'" onmouseout="this.style.background=\'{bg_color}\'">\n'
            html += f'<td style="padding: 8px; border-bottom: 1px solid #e0f2fe; color: #0c4a6e; font-weight: 500;">{name}</td>\n'
            ...
            arch_color = '#3b82f6' if '64-bit' in arch else '#8b5cf6'
            arch_badge = f'<span style="background: {arch_color}; color: white; padding: 3px 10px; border-radius: 12px; font-size: 0.75em; font-weight: 600;">{arch}</span>'
            html += f'<td style="padding: 8px; border-bottom: 1px solid #e0f2fe; text-align: center;">{arch_badge}</td>\n'
            html += '</tr>\n'
...
def generate_html_report(audit_data, remediation_data, output_file):
    ...
    # Section BIOS - Vérification manuelle
    html_content += '<div style="margin: 30px 40px; padding: 20px; background: #fff3cd; border-left: 4px solid #f59e0b; border-radius: 8px;">\n'
    html_content += '<h2 style="color: #92400e; margin-bottom: 12px; font-size: 1.2em; font-weight: 600;">🖥️ Vérification Manuelle Requise - BIOS/UEFI</h2>\n'
    html_content += '<p style="color: #78350f; font-size: 0.95em; margin: 8px 0; line-height: 1.6;">Recommandation pour le mot de passe BIOS .</p>\n'
    html_content += '<p style="color: #78350f; font-size: 0.95em; margin: 8px 0; line-height: 1.6;"><strong>⚠️ Action requise :</strong> Le mot de passe BIOS/UEFI doit être vérifié manuellement et mis en place sur chaque machine. Cette vérification ne peut pas être automatisée via un script.</p>\n'
    html_content += '</div>\n'
